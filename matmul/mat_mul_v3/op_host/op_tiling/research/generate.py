...
def main() -> None:
    ...
    if direct_deployment:
        print(
            ...
            "runtime_history=0 model=0 npu_candidate_pool=1 "
            "host_option_set=independent_template_formulas "
    ...
    for workload in workloads:
        ...
        if one_shot_decision is not None:
            ...
            print(
                ...
                f"{workload.workload_id} "
                f"template={selected.template.value} "
                "history_rows_used=0 npu_candidate_pool=1 "
                "formula=independent_base_plus_hardware_template_competition "
                "options_considered="
                f"{int(selected.metrics.get('rule_options_considered', 1))} "
                "selection_score="
                f"{selected.metrics.get('rule_selected_score', 0.0):.6g} "
                "bank_signature_exact="

...
RESULT_DIR="$ROOT_DIR/results/matmul_cost_filter_audit/$SOURCE_SIGNATURE/$RUN_ID"
LOG_DIR="$RESULT_DIR/logs"
BUILD_STATE_DIR="$ROOT_DIR/.benchmark_state/matmul_cost_filter_audit/$SOURCE_SIGNATURE"
...
export MATMUL_AUDIT_STATE_DIR="$RUN_STATE_DIR/search"
export MATMUL_AUDIT_ROUNDS="${MATMUL_AUDIT_ROUNDS:-4}"
export MATMUL_AUDIT_SEED="${MATMUL_AUDIT_SEED:-20260828}"
export MM_DATA_SEED="${MM_DATA_SEED:-20260828}"
export MM_REUSE_GOLDEN=1

printf 'MATMUL_FILTER_AUDIT_READY run_id=%s shape=512x512x512 device=%s rounds=%s logs=%s\n' \
    "$RUN_ID" "$PHYSICAL_DEVICE" "$MATMUL_AUDIT_ROUNDS" "$LOG_DIR"

if [[ ! -x "$PROJECT_DIR/ascendc_kernels_bbit" || ! -f "$BUILD_STATE_DIR/build_complete" ]]; then
